...
export function InfoAttributeView({
	...
	const { handleShowOnMap, handleDeleteFeature, handleSaveEdit, handleExportKML } =
		useFeatureActions(
			activeLayer,
			activeFeatureId,
			feature,
	...
	if (activeLayer.id === VIRT_MARKER_LAYER_ID) {
		visibleAtribs = visibleAtribs.filter(atrib => VIRT_MARKER_NEW_FIELDS.includes(atrib.name));
	}

	const layerName = activeLayer.get
		? activeLayer.get('descr')
		: (activeLayer.descr ?? 'Информация об объекте');
		...
		>
			...
			>
				<InfoAttributeHeader
					...
					disableNext={
						currentIndex === (allFeatures?.length || 1) - 1 || isGeometryEditing
					}
