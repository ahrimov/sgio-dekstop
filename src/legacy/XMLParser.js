import { updateAppMode } from "./initialLoad";
import { root_directory, pathToImageStorage, pathToKMLStorage } from './initial.js';
import { openFile } from "./FileManage";
import { setDBProgressCallbacks, setTotalLayersCount, initialDB, getDataLayerFromBD } from './DBManage.js';
...
export function configParser(data) {
    ...
    const layersName = dom.getElementsByTagName("LayersName").item(0).textContent.split("|").filter(el => Boolean(el));
    ...
    if(typeof pathToBaseRasterLayers !== "undefined"){
        openFile(root_directory + pathToBaseRasterLayers, function(text){
            try{
...
export async function pointStyleParse(dom){
    ...
    function parsePointStyleOld(dom) {
        const fill = new Fill({color: dom.getElementsByTagName("Fill").item(0).getElementsByTagName("CssParameter").item(0).textContent});
        const xmlStroke = dom.getElementsByTagName("Stroke").item(0);
        const stroke = new Stroke({color:xmlStroke.getElementsByTagName("CssParameter").item(0).textContent, width:parseInt(xmlStroke.getElementsByTagName("CssParameter").item(1).textContent)});
